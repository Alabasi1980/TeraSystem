...
## قرار السياق والتوكنز للمشروع

| البند | القرار |
|---|---|
| مستوى حساسية التوكنز | متوسط |
| السبب | توجد ملفات تحضيرية متعددة وعملاء مولدون، لكن المشروع صغير ومحدود |
| نوع السياق الافتراضي | Task Context + `project-control/PROJECT_STATE.md` |
| Full Context | ممنوع افتراضيًا إلا عند مراجعة شاملة مبررة |
| هل يلزم `PROJECT_STATE.md`؟ | نعم |
| هل يلزم `CONTEXT_INDEX.md` الآن؟ | لا، يؤجل حتى تكبر الملفات أو يتكرر الهدر |
| العملاء المتوقعون لاستهلاك توكنز أعلى | Tera عند Readiness Review، وEngineeringAgent لاحقًا عند التنفيذ |
| وضع التشغيل الافتراضي | Plan Mode |
| هل يسمح بـ Build Mode الآن؟ | لا، حتى اعتماد Readiness Review و TASK-ID |
| المهام التي تحتاج موافقة قبل التنفيذ | قراءة كل الملفات، نقل أو توليد عدة عملاء، تنفيذ كود، أو تشغيل أوامر مؤثرة |

### قواعد السياق لهذا المشروع

- يبدأ Tera من `project-control/PROJECT_STATE.md` ثم يقرأ الملفات المطلوبة فقط.
- لا يتم تمرير كامل `project-preparation/` لأي عميل فرعي إلا بموافقة صريحة.
- أي عميل فرعي يجب أن يتلقى قائمة `Reference Files` و`Allowed Write Targets`.
- المخرجات يجب أن تكون مختصرة ومباشرة، ولا تعيد شرح المشروع كاملًا.
