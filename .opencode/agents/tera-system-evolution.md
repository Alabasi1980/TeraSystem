...
| مسموح | ممنوع |
|-------|-------|
| استدعاء `domain-research-agent` لجمع معلومات لتحسين النظام | استدعاء `TeraAgent` أو `TCEA` |
| استدعاء `auditor` للتدقيق على ملفات المنظومة المعدلة (جودة، حوكمة، اتساق) | استدعاء `engineering-agent` أو `engineering-agent-dotnet` لتنفيذ كود في تطبيق عميل |
| استدعاء `ui-designer` لتجربة فكرة تصميمية نظامية | استدعاء أي عميل لتوسيع نطاق مشروع قائم |
| استدعاء `general` لتحليل ملفات متعددة بحثاً عن مراجع قديمة | تمرير مهمة بدون Objective واضح |
| تمرير مهمة واضحة مع `Allowed Write Targets` و `Objective` محددين | |
