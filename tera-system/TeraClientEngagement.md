...
## 11. نظام التسعير التجاري — Pricing System (v4.2 — معتمدة)
...
- **المرجعية**: v4.2 — معتمدة — تُراجع وتُقيَّم بعد أول 5 مشاريع تجارية.

### 11.2 مرجع لسياسة التسعير
مصدر الحقيقة الوحيد: `tera-system/TeraPricingPolicy.md`

هذا القسم يلخص النقاط التشغيلية. التفاصيل الكاملة (Rubric الـ 12 عاملاً، الأوزان، المضاعفات، أمثلة الاحتساب) في ملف السياسة.
...
### 11.4 معادلة التسعير الداخلية (v4.2)
```
...
Minimum Profitable Price = Estimated Hours × 4 JOD
Approved Price = max(Final Scorecard Price, 100 JOD, Minimum Profitable Price)
```

### 11.5 آلية احتساب Complexity Index
- **12 عاملاً** للتقييم (شاشات، منطق أعمال، DB، صلاحيات، تقارير، تكاملات، تصميم، طباعة، فروع، نشر، تدريب، اختبار).
- كل عامل يُقيَّم من **0 إلى 5** مع Rubric تفصيلي في السياسة.
- لكل عامل **وزن** (1.0 - 2.0) — المجموع الأقصى = 92.5.
- ```text
  Weighted Score = Σ (Score_i × Weight_i)
  Complexity Index % = (Weighted Score ÷ 92.5) × 100
  ```

### 11.6 Anchor Points — استخراج Base Price
...
- المشاريع فوق 80% لا تُسعّر مباشرة — تتطلب SOW أو دراسة تفصيلية.

### 11.7 الحد الأدنى للسعر
- **Minimum Engagement Fee**: 100 JOD — لا يُقبل مشروع بأقل من ذلك.
- **Minimum Profitable Price**: الساعات المقدرة × 4 JOD/ساعة.
- السعر المعتمد = **الحد الأعلى** من (Final Scorecard Price, 100 JOD, Minimum Profitable Price).

### 11.8 Discovery / Paid Analysis

| حجم المشروع | السعر (JOD) |
|------------|-----------:|
| صغير | 35 - 75 |
| متوسط | 100 - 200 |
| كبير | 250 - 500 |
...
- يمكن خصم 50% من سعر التحليل من سعر التنفيذ (بشروط: اعتماد خلال 14 يوماً، نطاق غير متغير، قيمة ≥ 400 JOD، لا يكسر الحد الأدنى).

### 11.9 Risk Margin و Rush Premium
...
| عالي جداً | نطاق غير مستقر أو تكاملات غامضة | 20% |
...
| ضغط خفيف (20%-30%) | 10% |
| ضغط واضح (31%-50%) | 20% |
| ضغط حاد (>50%) | 35% أو رفض السعر المقطوع |

### 11.10 قواعد الصيانة
- 3 أشهر Bug Fix Warranty مجاناً.
- بعدها: Maintenance Subscription (20-300 JOD/شهر حسب حجم المشروع).
- بدون اشتراك: يُسعّر لكل حالة.
- الضمان لا يشمل: ميزات جديدة، تغيير متطلبات، سوء استخدام، مشاكل استضافة خارجية.

### 11.11 قواعد طلبات التغيير
6 تصنيفات: Minor Refinement (مجاني داخل جولات المراجعة)، Structural Change (مُسعّر)، Scope Increase (مُسعّر)، Client-Driven Rework (مُسعّر)، Bug Fix (ضمن الضمان)، Post-Delivery (ضمن صيانة أو لكل حالة).
...
- **يُستخدم قالب الخطاب الرسمي:** `letterhead-master-fixed-print.html`.
...
```text
TCEA يصدر Draft Quotation فقط (مسودة).
...
```
1. تأكيد توفر معلومات النطاق الـ 14 (Section 10.3 في تعريف TCEA).
2. فتح حاسبة Excel: project-control/TeraPricingCalculator.xlsx.
3. تقييم العوامل 12 (0-5) باستخدام Rubric السياسة.
4. اختيار Platform Multiplier ولغة و Risk Margin و Rush Premium من القوائم.
5. إضافة Add-ons الخارجية إن وجدت.
6. إدخال الساعات المقدرة للتنفيذ.
7. قراءة "السعر المعتمد النهائي" من الحاسبة.
8. تطبيق قائمة الاعتماد (14 سؤالاً) — في تعريف TCEA §10.6.
9. التحقق من Quotation Readiness Gate.
10. إنتاج DRAFT_QUOTATION.md ← Majed للمراجعة والاعتماد.
```
...
- **Level 2**: بعد توثيق النطاق + استخدام الحاسبة + `Quotation Readiness Gate` — مسودة.
