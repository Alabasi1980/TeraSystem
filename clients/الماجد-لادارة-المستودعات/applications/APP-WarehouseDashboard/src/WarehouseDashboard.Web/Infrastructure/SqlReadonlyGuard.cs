...
/// <summary>
/// Server-side guard that ensures a SQL batch is read-only before it is executed
/// by the Admin Query Tester. Defence in depth: the UI also prevents writes, but
/// the executing endpoint must never trust the client.
///
/// Rules enforced:
/// - Every statement in the batch must be a <c>SELECT</c> or a <c>WITH</c> (CTE)
///   that leads to a <c>SELECT</c>.
/// - Leading comments (<c>--</c> line comments and <c>/* */</c> block comments)
///   are tolerated and stripped before analysis.
/// - Data-modifying and DDL keywords are rejected with a clear Arabic message.
/// - <c>SELECT ... INTO</c> (table creation via SELECT) is also rejected.
/// </summary>
public static class SqlReadonlyGuard
{
    // First-word keywords that are never allowed in the read-only tester.
    private static readonly HashSet<string> BlockedKeywords = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        "EXEC", "EXECUTE", "SP_EXECUTESQL", "GRANT", "REVOKE", "DENY", "BACKUP",
        "RESTORE", "BULK", "USE", "SHUTDOWN", "KILL", "INTO"
    };

    /// <summary>
    /// Returns <c>true</c> when the batch is read-only; otherwise <paramref name="reason"/>
    /// contains a localized, user-safe explanation.
    /// </summary>
    public static bool IsReadOnly(string sql, out string reason)
    {
        ...
        {
            reason = "الرجاء إدخال استعلام SQL.";
            return false;
        }

        // Remove comments and blank out string/bracket literals so their contents
        // (which may contain keywords or semicolons) cannot spoof the analysis.
        ...
        {
            reason = "الرجاء إدخال استعلام SQL صالح.";
            return false;
        }
        ...
        foreach (var statement in statements)
        {
            ...
            else if (firstWord.Equals("WITH", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    reason = "استعلام WITH يجب أن يؤدي إلى SELECT (للقراءة فقط فقط).";
                    return false;
                }
            }
            else
    ...
    /// and bracket identifiers (<c>[...]</c>) with spaces, preserving structure so
    /// keyword and statement-boundary detection is reliable.
    /// </summary>
    private static string Normalize(string sql)
    {
        ...
        for (int i = 0; i < sql.Length; i++)
        {
            ...
            if (inBracket)
            {
                ...
                continue; // consume identifier content
            }
