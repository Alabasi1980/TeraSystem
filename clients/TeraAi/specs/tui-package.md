...
```text
packages/opencode ---\
                      > @tera-system/tui -> @tera-system/sdk
packages/cli --------/
```
...
`packages/cli`, or `@tera-system/core`.
...
- Do not replace private `packages/opencode` imports with `@tera-system/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@tera-system/tui`.

## Ownership Boundary

### `@tera-system/tui` Owns
...
Status: Completed. The private `@tera-system/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@tera-system/tui`.
...
- The package has no dependency on `opencode`, `@tera-system/cli`, or
  `@tera-system/core`.
...
primitives now live in `@tera-system/tui`. The legacy host consumes explicit
...
and build inputs through `@tera-system/tui/runtime`. Movable app, component,
...
`@tera-system/tui/config` now owns schemas, defaults, keybind resolution, the
...
canonical in `@tera-system/tui`. Configured references resolve through the new
...
`@tera-system/tui`. The legacy host injects a narrow plugin host that retains
...
public `@tera-system/tui` root while retaining worker/server/config/plugin and
...
- Narrow `@tera-system/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@tera-system/core`, or either
  executable package.
...
rg '@tera-system/core|packages/opencode|packages/cli' packages/tui
