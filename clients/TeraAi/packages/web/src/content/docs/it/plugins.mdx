...
I plugin ti permettono di estendere OpenCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalitأ , integrare servizi esterni o modificare il comportamento predefinito di OpenCode.
...
I plugin vengono caricati da tutte le sorgenti e tutti gli hook vengono eseguiti in sequenza. L'ordine di caricamento أ¨:
...
Un plugin أ¨ un **modulo JavaScript/TypeScript** che esporta una o piأ¹ funzioni di plugin. Ogni funzione riceve un oggetto di contesto e restituisce un oggetto di hook.
...
:::note
Se usi l'app desktop di OpenCode, puأ² inviare automaticamente notifiche di sistema quando una risposta أ¨ pronta o quando una sessione va in errore.
:::
...
L'helper `tool` crea uno strumento personalizzato che opencode puأ² chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
Quando `output.prompt` أ¨ impostato, sostituisce completamente il prompt di compaction predefinito. In questo caso l'array `output.context` viene ignorato.
