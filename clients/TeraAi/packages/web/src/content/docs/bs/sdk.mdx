...
Moإ¾ete zatraإ¾iti strukturirani JSON izlaz od modela specificiranjem `format` sa JSON إ،emom. Model ؤ‡e koristiti `StructuredOutput` alat da vrati validirani JSON koji odgovara vaإ،oj إ،emi.
...
| `json_schema` | Vraؤ‡a validirani JSON koji odgovara pruإ¾enoj إ،emi                   |
...
| Polje        | Tip             | Opis                                                        |
| ------------ | --------------- | ----------------------------------------------------------- |
| `type`       | `'json_schema'` | Obavezno. Odreؤ‘uje JSON schema naؤچin rada                   |
| `schema`     | `object`        | Obavezno. JSON Schema objekt koji definira strukturu izlaza |
| `retryCount` | `number`        | Opcionalno. Broj ponovnih pokuإ،aja validacije (default: 2)  |

### Rukovanje greإ،kama

Ako model ne uspije proizvesti validan strukturirani izlaz nakon svih ponovnih pokuإ،aja, odgovor ؤ‡e ukljuؤچivati `StructuredOutputError`:
...
1. **Navedite jasne opise** u svojstvima vaإ،e إ،eme kako biste pomogli modelu da razumije koje podatke treba izdvojiti
2. **Koristite `required`** da odredite koja polja moraju biti prisutna
3. **Drإ¾ite إ،eme fokusiranim** - sloإ¾ene ugnijeإ¾ؤ‘ene إ،eme modelu mogu biti teإ¾e za ispravno popunjavanje
4. **Postavite odgovarajuؤ‡i `retryCount`** - poveؤ‡ajte za sloإ¾ene إ،eme, smanjite za jednostavne

---

## API-ji

SDK izlaإ¾e sve server API-je kroz type-safe klijent.
