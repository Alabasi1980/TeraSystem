...
- Author package entrypoint is `@tera-system/plugin/tui`.
...
- Import types from `@tera-system/plugin/tui`.
- `@tera-system/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@tera-system/plugin/tui"
...
- `opencode plugin <module>` and TUI install both run install â†’ manifest read â†’ config patch.
...
When `plugin` entries exist in a writable `.opencode` dir or `OPENCODE_CONFIG_DIR`, OpenCode installs `@tera-system/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@tera-system/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@tera-system/plugin/tui`.
